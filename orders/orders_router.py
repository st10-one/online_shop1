...
@orders_router.post("")
def make_order(data:Orders, user:dict = Depends(get_active_user)):
...
def cancel_order(order_id:int, user:dict = Depends(get_active_user)):
...
@orders_router.get('')
def get_my_orders(user:dict = Depends(get_active_user)):
